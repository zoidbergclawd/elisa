...
Rules are guardrails that tell your AI agents what they must (or must not) do. Unlike [skills](Skills), which add capabilities, rules add constraints.

## What Rules Do

Rules fire automatically at specific points during a build. For example, a rule with the trigger "before deploy" will be checked right before your project is deployed — you don't have to remember to run it.

Rules are attached to your project by dragging an "Apply Rule" [block](Block-Reference#rules) onto the canvas.
...
- **Before deploy** rules are checked during the deploy phase. Before-deploy rules are surfaced as a `deploy_checklist` event.
