# IoT Sensor Network Guide

Build a real IoT sensor network with Elisa! This guide walks you through the whole thing -- from plugging in wires to watching live sensor data on a cloud dashboard.

Your finished project will have two devices talking to each other wirelessly:

- A **Sensor Node** that reads temperature, humidity, door open/close, and motion
- A **Gateway Node** that receives the data over LoRa (like a walkie-talkie for devices!) and sends it to the internet
- A **Cloud Dashboard** where you can watch your sensor data update live from anywhere
...
## Building Your Project in Elisa
...
Open the **IoT Devices** category in the toolbox. Drag an **IoT Sensor Node** block onto the canvas.
...
- Set the **LoRa channel** (any number from 0-7, just remember it -- the Gateway must match)
...
Drag an **IoT Gateway Node** block onto the canvas.
...
Drag an **IoT Cloud Dashboard** block onto the canvas.

- Type your **GCP project ID** (see [Cloud Run Setup](#cloud-run-setup) below if you have not set this up yet)

### Step 5 (optional): Add a guide

Open the **Goals** category. Drag a **Write me a guide** block onto the canvas.

Pick what the guide should cover:
- "how everything works" -- explains the code and architecture
- "how to set it up" -- setup and installation steps
- "what each part does" -- sensor and component descriptions
- "all of the above" -- everything

The AI minions will write a custom guide about your specific project after the build finishes.
...
That is it! Use the project ID you created (like `my-iot-dashboard`) in the IoT Cloud Dashboard block in Elisa.
