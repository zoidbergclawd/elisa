...
- **SDK query per task**: Each agent task calls `query()` from `@anthropic-ai/claude-agent-sdk` with `permissionMode: 'bypassPermissions'` and explicit `pathToClaudeCodeExecutable` (resolved once at module load by `agentRunner.ts`). When no system Node.js is available, Electron main process sets `ELECTRON_RUN_AS_NODE=1` and creates a `node.exe` shim (hardlink of Elisa.exe); agentRunner detects this and passes `executable: process.execPath` to the SDK. When system node exists, the SDK uses it directly (preserving npm/npx). Default `maxTurns=25` (`MAX_TURNS_DEFAULT`). On retry, grants 10 additional turns per attempt (`MAX_TURNS_RETRY_INCREMENT`), so retries progress: 25 → 35 → 45.
