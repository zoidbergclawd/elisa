  ...
  async deployDevices(
    ...
    for (const device of order) {
      ...
      if (manifest.deploy.method === 'cloud') {
        ...
        // Check if agent generated the entry point files; use plugin template as fallback
          ...
          for (const entryFileName of flashConfig.files) {
            ...
            if (!fs.existsSync(workspaceFile)) {
              ...
              if (fs.existsSync(templateFile)) {
                ...
                log(`  WARNING: agent did not generate ${entryFileName} -- using plugin template`);
                await ctx.send({
                  type: 'deploy_progress',
                  device_role: device.pluginId,
                  step: `Warning: ${entryFileName} not generated by agent, using default template`,
                  progress: 25,
                });
              } else {
                log(`  ERROR: ${entryFileName} missing and no template fallback at ${templateFile}`);
                await ctx.send({
                  type: 'error',
                  message: `Agent failed to generate ${entryFileName} for ${manifest.name} and no template exists`,
                  recoverable: true,
                });
              }
            }
