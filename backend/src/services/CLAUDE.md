...
Registers the `doc-agent` meeting type with `canvasType: 'explain-it'`. Triggers on `plan_ready` (mid-build, alongside Media Agent). Agent persona is "Scribe". Frontend canvas: `ExplainItCanvas.tsx`. `registerDocAgentMeeting(registry)` called at startup.
