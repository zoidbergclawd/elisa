...
- **promptBuilder.ts** -- Prompt construction for agent tasks. Assembles system prompt (role template, NuggetSpec, skills, digests), predecessor summaries, device plugin context, framework-specific context (via `getFrameworkPrompt()`), and MCP server config. Extracted from executePhase.ts for single-responsibility.
...
- **deployPhase.ts** -- Web preview (built-in static file server via `staticServer.ts`), device flash (via plugin manifests), CLI portal execution
...
Claude-powered agent responses for meeting conversations. Uses Anthropic SDK (Haiku model via `NARRATOR_MODEL_DEFAULT` constant). Calls `getAnthropicClient()` per-use (no cached `this.client`) so API key changes propagate immediately. Dual-call architecture: `generateResponse()` makes two parallel API calls via `Promise.allSettled` -- a chat call (persona + build context + "NEVER output JSON", `MEETING_CHAT_MAX_TOKENS=300`) and a canvas call (canvas schema + "Output ONLY a valid JSON object", `MEETING_CANVAS_MAX_TOKENS=4096`). Canvas call skipped when no `CANVAS_INSTRUCTIONS` entry exists for the meeting's `canvasType`. Optional `options` parameter: `focusContext` (scopes prompts to a specific task) and `previousDesigns` (lists already-designed elements for consistency). When provided, chat prompt gets "Your Focus" + "Already Designed" sections; canvas prompt gets "Scope" + "Previously Designed" sections. Chat prompt for Blueprint includes "Build Health" (grade+breakdown), "Failing Tests" (up to 5 with error details), and "Architecture" (complexity, inputs/outputs) sections when data available. `MeetingBuildContext` carries `healthBreakdown`, `complexity`, `heaviestRequirements`, `systemInputs`, `systemOutputs`, and `testResults` with optional `details`. `parseCanvasResponse()` strips accidental fencing then tries `JSON.parse` with `sanitizeJsonStrings` fallback for literal newlines in draw code. Each call fails gracefully: chat failure returns fallback text, canvas failure returns `undefined`. Converts `MeetingMessage[]` to Claude format (kid->user, agent->assistant). 45s timeout via `withTimeout()` (`MEETING_AGENT_TIMEOUT_MS`), canvas timeout logged as `console.warn` (expected under load), chat timeout logged as `console.error`.
...
Registers the `architecture-agent` meeting type with `canvasType: 'blueprint'`. Triggers on `session_complete` (post-build, after tests and health summary). Agent persona is "Blueprint". Receives full build summary data: health grade+breakdown, test results with error details, architecture complexity, system inputs/outputs. Frontend canvas: `BlueprintCanvas.tsx`. `registerArchitectureAgentMeeting(registry)` called at startup.
...
Orchestrator.run(spec)
  ...
  |-> PlanPhase.execute(ctx, spec)      returns tasks, agents, DAG, framework
  |-> copyFrameworkToWorkspace()        copy selected lib (Phaser/p5/Three.js) to workspace
