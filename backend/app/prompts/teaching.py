"""Prompt templates and curriculum for the teaching engine."""
...
def get_curriculum_moment(concept: str, sub_concept: str) -> dict | None:
    """Look up a teaching moment from the hardcoded curriculum.

    Returns a dict with concept, headline, explanation, tell_me_more
    or None if not found.
    """
    category = CONCEPT_CURRICULUM.get(concept)
    if not category:
        return None
    return category.get(sub_concept)
...
def teaching_user_prompt(event_type: str, event_details: str, project_type: str = "software") -> str:
    """Format a user prompt for the API fallback path."""
    return (
        f"A kid is watching their {project_type} project being built by AI agents. "
        f"The following event just happened: {event_type}. "
        f"Details: {event_details}. "
        f"Generate a short, kid-friendly teaching moment about this. "
        f"Respond as JSON with keys: concept, headline, explanation, tell_me_more"
    )
